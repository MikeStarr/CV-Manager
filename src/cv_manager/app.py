...
def main():
    ...
    with container:
        ...
        st.markdown("<p style='text-align:center;'>Analyze job specifications, identify keyword gaps, and surgically tailor your CV templates.</p>", unsafe_allow_html=True)
    ...
    with col_right:
        ...
        with tab_ats:
            if not job_spec.strip():
                ...
                st.info("Please verify your LLM settings (API Base URL, API Key, Model Name) in the sidebar configuration.")
            else:
        ...
        with tab_tailor:
            ...
            if st.session_state.get('tailored_success_msg'):
                ...
                if remaining_gaps:
                    st.warning(f"The tailored CV still lacks mentions of the following {len(remaining_gaps)} ATS keywords. You may need to address these manually or discuss them as transferable skills:")
                    st.markdown(" ".join(f"`{g}`" for g in remaining_gaps))
                else:
                    st.success("🎉 All ATS keywords have been successfully matched and incorporated into the tailored CV!")
            else:
